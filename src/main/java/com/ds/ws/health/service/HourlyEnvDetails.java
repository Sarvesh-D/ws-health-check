...
public class HourlyEnvDetails extends FetchDetailsFromReport {
	...
	int getFirstRowNum() {
		int firstRowNum = 0;
		int totalPings = (reportUtils.getPings()-1) - getPingsForStatusCalc(); // subtracting the server startup ping 
		logger.debug("Total Pings [{}]", totalPings);
		if(totalPings <= 0) firstRowNum = 1; // 0 is table header, starting from row 1
		else firstRowNum = 1 + (totalPings * wsHealthUtils.getAllServices().size());
	...
	int getLastRowNum() {
		...
		int totalPings = reportUtils.getPings() - 1; // subtracting the server startup ping
		logger.debug("Total Pings [{}]", totalPings);
